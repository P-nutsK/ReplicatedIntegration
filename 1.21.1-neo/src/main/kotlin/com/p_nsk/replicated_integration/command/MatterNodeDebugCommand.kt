...
object MatterNodeDebugCommand {
    ...
    private fun describeNode(source: CommandSourceStack, node: NodeKey): Int {
        ...
        val registry = NeoReplicationCalculationService.matterNodeRegistry()
        ...
        source.sendSuccess({ buildSummary(node, compound, registry) }, false)
    ...
    private fun traceNode(source: CommandSourceStack, node: NodeKey, depth: Int): Int {
        ...
        emitTrace(source, node, depth, 0, graph, solved, seen, registry)
        return 1
    }
            ...
            .forEach { conversion ->
                ...
                if (depth > 1) {
                    conversion.consumes.forEach { consumed ->
                        emitTrace(source, consumed.node, depth - 1, indent + 2, graph, solved, seen, registry)
                    }
                }
            }
    }
