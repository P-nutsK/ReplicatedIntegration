...
object ForgeRecipeConversionSupport {
    ...
    fun addConversion(
        ...
        produces: List<MatterAmount>,
        collector: IConversionSink,
    ) {
        if (consumes.isEmpty() || produces.isEmpty()) {
            return
        }
        collector.add(
            MatterConversion(
    ...
    fun addConversionsForAlternatives(
        ...
        if (collapsed.size == 1 && collapsed.single().first.size == 1) {
            ...
            addConversion(id, listOf(MatterAmount(only.node, only.amount * multiplier)), produces, collector)
            return true
        }
        ...
        fun visit(depth: Int, selected: MutableList<MatterAmount>) {
            if (depth == collapsed.size) {
                ...
                addConversion(conversionId, selected.toList(), produces, collector)
                index++
                return
            }
