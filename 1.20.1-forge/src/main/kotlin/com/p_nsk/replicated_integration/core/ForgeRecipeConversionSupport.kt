...
object ForgeRecipeConversionSupport {
    ...
    fun addConversion(
        ...
        // when simple enough, avoid the overhead of generating multiple conversions
        if (collapsed.size == 1) {
        ...
        fun visit(depth: Int, selected: MutableList<NodeAmount> = mutableListOf()) {
            if (depth == collapsed.size) {
                val conversionId = if (index == 0) {
                    id
                } else {
                    ResourceLocation.fromNamespaceAndPath(
                        id.namespace,
                        "${id.path}/alternative_${index.toString().padStart(suffixWidth, '0')}",
                    )
                }
    ...
    fun ingredientToInputNode(
        ...
        val key = ingredient.items.joinToString("|") { stack ->
            "${BuiltInRegistries.ITEM.getKey(stack.item)}#${stack.count.coerceAtLeast(1)}"
        }

        val cached = ingredientInputNodesCache.getOrPut(key) {
            ingredient.items.mapNotNull { stack ->
                val node = nodeOf(stack) ?: return@mapNotNull null
                val amount = stack.count.coerceAtLeast(1).toLong()
                NodeAmount(node, amount)
            }.let(::InputNodes)
        }

        return if (amountOverride != null) {
            InputNodes(
                cached.map { nodeAmount ->
                    nodeAmount.copy(amount = amountOverride)
                }
            )
        } else {
            cached
    ...
    ): List<Pair<InputNodes, Long>> = inputs
        .groupingBy { it }
        .eachCount()
