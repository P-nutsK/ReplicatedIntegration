...
object MatterCommandTree {
    ...
    private fun commandTargets(): List<ForgeMatterCommand> =
        ForgeReplicationCalculationService.commandTargets()
    ...
    private fun getBranch() =
        Commands.literal("get").also { branch ->
            ...
            // debug / escape hatch. 書き込み系には出さない。
    ...
    private fun targetGetSelector(target: ForgeMatterCommand): ArgumentBuilder<CommandSourceStack, *> =
    ...
    private fun targetSetSelector(target: ForgeMatterCommand): ArgumentBuilder<CommandSourceStack, *> =
        Commands.literal(target.literal)
            .then(
                Commands.argument("id", ResourceLocationArgument.id())
                    .suggests(target.suggestions)
